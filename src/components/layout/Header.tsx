      ...
      >
        <div className="container mx-auto px-4 sm:px-6 max-w-7xl">
          <div className="flex justify-between items-center py-4">
            ...
            {/* Desktop Navigation */}
            <nav className="hidden gap-6 lg:gap-8 text-base font-normal lg:flex font-source-sans" >
              {navItems.map((item) => (
                <a
                  key={item.label}
                  href={item.href}
                  onClick={(e) => handleClick(e, item.href)}
                  className="relative transition-colors hover:text-aws-orange cursor-pointer whitespace-nowrap group"
                >
                  {item.label}
                  <span className="absolute left-0 -bottom-1 w-0 h-[1px] bg-aws-orange transition-all duration-300 group-hover:w-full"></span>
                </a>
              ))}
            </nav>
            ...
            <button
              ...
              className="lg:hidden relative z-50 p-3 -mr-2 text-white hover:text-aws-orange transition-colors min-w-[44px] min-h-[44px] flex items-center justify-center"
